...
% Set defaults
trials = 5;
csv_dir = '/home/dinesh/Dropbox/Science/Projects/Antennal-Positioning/Simulation/Antennal-motor-system-neural-network/Simulated-Data/Extrinsic-setpoint';
...
wsNames = {'ws_0_mps'
    'ws_1_5mps'
    'ws_2_5mps'
    'ws_4_mps'};    % Mimicing different set-points as airflow (for now)
...
for trt = 1:length(treatment_name)
    ...
    for i=1:length(csv_files)
        ...
        [~,tok] = regexp(csv_files{i},...
            'In-([\d \.]*)_T-(\d*).csv', 'match','tokens');
        trial_no = str2double(tok{1}{2});
        windspeed = wsNames{ismember(in_asyms, str2double(tok{1}{1}))};
...
%% Predict return curves (add it as batch jobs)

for i=1:length(treatment_name)
%     [ModelFits, ModelData, ExpFits] = analyzeReturnCurves(matfolder, outfolder,...
%         treatment_name, angle_name, filter_vals);
    batch('predictAntennalReturnCurves', 0, {matfolder, outfolder,...
        treatment_name(i), angle_name, filter_vals});
end